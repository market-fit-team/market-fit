import type {
  OnboardingAreaRecommendationResult,
  OnboardingSurvey,
  OnboardingSurveyResult,
} from "@/features/onboarding/types/onboarding"

export const onboardingSurveyFixture: OnboardingSurvey = {
  id: 1,
  slug: "founder-fit-12-final",
  version: 1,
  survey_code: "A",
  scoring_version: "founder_fit_v2",
  title: "창업 성향 설문 12문항",
  description: "업종 추천과 상권 추천을 함께 만들기 위한 설문",
  question_count: 12,
  questions: [
    {
      id: "q1",
      selection_type: "single",
      prompt: "내 가게에 가장 자주 왔으면 하는 손님은 어떤 분인가요?",
      max_selections: null,
      options: [
        { code: "A", label: "집 근처라 자주 오는 동네 주민" },
        { code: "B", label: "점심이나 퇴근길에 들르는 직장인" },
        { code: "C", label: "일부러 찾아오는 목적형 손님" },
        { code: "D", label: "주말에 가족이나 연인과 오는 손님" },
      ],
      primary_parameters: ["resident_focus_level"],
      secondary_parameters: ["worker_focus_level"],
    },
    {
      id: "q2",
      selection_type: "single",
      prompt: "가게가 가장 바빴으면 하는 시간대는 언제인가요?",
      max_selections: null,
      options: [
        { code: "A", label: "평일 점심" },
        { code: "B", label: "평일 저녁" },
        { code: "C", label: "주말 낮" },
        { code: "D", label: "주말 저녁" },
      ],
      primary_parameters: ["weekend_preference_level"],
      secondary_parameters: ["evening_preference_level"],
    },
    {
      id: "q3",
      selection_type: "single",
      prompt: "자리 고를 때 지하철역과의 거리는 얼마나 중요하다고 느끼시나요?",
      max_selections: null,
      options: [
        { code: "A", label: "역이 가까워야 거의 안심이 된다" },
        { code: "B", label: "가까우면 좋지만 절대 조건은 아니다" },
        { code: "C", label: "크게 신경 쓰지 않는다" },
        { code: "D", label: "골목이나 생활권 안쪽도 괜찮다" },
      ],
      primary_parameters: ["subway_dependency_level"],
      secondary_parameters: ["stability_level"],
    },
    {
      id: "q4",
      selection_type: "single",
      prompt: "창업 초기에 가장 중요하게 생각하는 것은 무엇인가요?",
      max_selections: null,
      options: [
        { code: "A", label: "초기 투자금을 최대한 낮게 시작하는 것" },
        { code: "B", label: "투자금이 들더라도 좋은 자리를 잡는 것" },
        { code: "C", label: "초반 수익이 늦어도 버틸 여유를 남기는 것" },
        { code: "D", label: "빠르게 매출을 만들고 회수하는 것" },
      ],
      primary_parameters: ["budget_level"],
      secondary_parameters: ["competition_tolerance_level"],
    },
    {
      id: "q5",
      selection_type: "single",
      prompt: "같은 돈을 쓴다면 어디에 더 쓰고 싶으신가요?",
      max_selections: null,
      options: [
        { code: "A", label: "월세와 보증금 부담을 낮추는 데" },
        { code: "B", label: "월세가 높아도 더 좋은 위치를 잡는 데" },
        { code: "C", label: "운영자금까지 남겨두는 데" },
        { code: "D", label: "초반 홍보와 빠른 자리 잡기에" },
      ],
      primary_parameters: ["rent_sensitivity_level"],
      secondary_parameters: ["budget_level"],
    },
    {
      id: "q6",
      selection_type: "single",
      prompt: "창업 후 1~2년 동안 어떤 흐름이 더 마음 편할 것 같나요?",
      max_selections: null,
      options: [
        { code: "A", label: "크게 안 벌어도 꾸준히 유지되는 흐름" },
        { code: "B", label: "변동이 있어도 성장 가능성이 큰 흐름" },
        { code: "C", label: "위험은 적지만 확장성도 크지 않은 흐름" },
        { code: "D", label: "리스크가 있더라도 빨리 커질 수 있는 흐름" },
      ],
      primary_parameters: ["stability_level"],
      secondary_parameters: ["competition_tolerance_level"],
    },
    {
      id: "q7",
      selection_type: "single",
      prompt: "경쟁이 많은 상권을 보면 어떤 생각이 드나요?",
      max_selections: null,
      options: [
        { code: "A", label: "이미 수요가 검증된 곳이라 오히려 괜찮다" },
        { code: "B", label: "굳이 그 안에서 싸우고 싶지는 않다" },
        { code: "C", label: "내가 더 잘할 수 있다면 들어갈 수 있다" },
        { code: "D", label: "상황을 조금 더 지켜본 뒤 판단하고 싶다" },
      ],
      primary_parameters: ["competition_tolerance_level"],
      secondary_parameters: ["stability_level"],
    },
    {
      id: "q8",
      selection_type: "single",
      prompt: "내가 생각하는 좋은 상권은 어떤 모습에 더 가깝나요?",
      max_selections: null,
      options: [
        { code: "A", label: "아파트와 빌라가 많아 생활 수요가 보이는 곳" },
        { code: "B", label: "오피스 건물이 많아 점심과 퇴근 수요가 보이는 곳" },
        { code: "C", label: "역세권이라 유동인구가 계속 흐르는 곳" },
        { code: "D", label: "너무 한쪽으로 치우치지 않은 혼합형 상권" },
      ],
      primary_parameters: ["resident_focus_level"],
      secondary_parameters: ["worker_focus_level"],
    },
    {
      id: "q9",
      selection_type: "single",
      prompt: "장사가 안 되는 날, 어떤 상황이 가장 불안할 것 같나요?",
      max_selections: null,
      options: [
        { code: "A", label: "평일 점심에 직장인 손님이 안 오는 것" },
        { code: "B", label: "저녁 시간인데 가게가 한산한 것" },
        { code: "C", label: "주말인데 생각보다 조용한 것" },
        { code: "D", label: "며칠 연속으로 전체 흐름이 꺾이는 것" },
      ],
      primary_parameters: ["worker_focus_level"],
      secondary_parameters: ["weekend_preference_level"],
    },
    {
      id: "q10",
      selection_type: "single",
      prompt: "운영 리듬을 고른다면 어느 쪽이 더 잘 맞나요?",
      max_selections: null,
      options: [
        { code: "A", label: "점심 회전이 빠른 운영" },
        { code: "B", label: "저녁 매출 비중이 높은 운영" },
        { code: "C", label: "주말 중심 운영" },
        { code: "D", label: "심야까지 이어지는 운영" },
      ],
      primary_parameters: ["lunch_preference_level"],
      secondary_parameters: ["late_night_preference_level"],
    },
    {
      id: "q11",
      selection_type: "single",
      prompt: "어떤 고객층이 더 잘 맞는다고 느끼시나요?",
      max_selections: null,
      options: [
        { code: "A", label: "10대~20대 중심" },
        { code: "B", label: "20대~30대 중심" },
        { code: "C", label: "30대~40대 중심" },
        { code: "D", label: "40대 이상 중심" },
      ],
      primary_parameters: ["target_age_20_level"],
      secondary_parameters: ["target_age_30_level"],
    },
    {
      id: "q12",
      selection_type: "multi",
      prompt: "자영업을 통해 이루고 싶은 것을 골라주세요.",
      max_selections: 3,
      options: [
        { code: "A", label: "안정적인 생활 기반" },
        { code: "B", label: "높은 수익과 확장 가능성" },
        { code: "C", label: "자유로운 시간과 운영 방식" },
        { code: "D", label: "동네에서 오래 인정받는 가게" },
        { code: "E", label: "내 감각이 드러나는 브랜드" },
      ],
      primary_parameters: ["stability_level"],
      secondary_parameters: ["competition_tolerance_level"],
    },
  ],
}

export const onboardingResultFixture: OnboardingSurveyResult = {
  survey: {
    id: 1,
    slug: "founder-fit-12-final",
    version: 1,
    survey_code: "A",
    scoring_version: "founder_fit_v2",
    title: "창업 성향 설문 12문항",
    description: "업종 추천과 상권 추천을 함께 만들기 위한 설문",
    question_count: 12,
  },
  result_code: "r3a3lz4o0x8w1f2j",
  profile_name: "설문 결과 프로필",
  share_path: "/onboarding/result/r3a3lz4o0x8w1f2j",
  share_url: "http://localhost:3000/onboarding/result/r3a3lz4o0x8w1f2j",
  area_user_profile: {
    user_id: "survey_a_area",
    profile_name: "설문 결과 프로필",
    budget_level: 0.28,
    stability_level: 0.82,
    subway_dependency_level: 0.11,
    weekend_preference_level: 0.72,
    evening_preference_level: 0.36,
    resident_focus_level: 0.84,
    worker_focus_level: 0.15,
    rent_sensitivity_level: 0.74,
    competition_tolerance_level: 0.19,
  },
  category_user_profile: {
    user_id: "survey_a_category",
    profile_name: "설문 결과 프로필",
    target_category_code: null,
    stability_level: 0.81,
    competition_tolerance_level: 0.32,
    weekend_preference_level: 0.74,
    lunch_preference_level: 0.38,
    evening_preference_level: 0.42,
    late_night_preference_level: 0.17,
    target_age_10_level: 0.06,
    target_age_20_level: 0.28,
    target_age_30_level: 0.31,
    target_age_40_level: 0.23,
    target_age_50_plus_level: 0.12,
    female_preference_level: 0.63,
    avg_ticket_preference: 0.57,
    traffic_volume_preference: 0.66,
    franchise_affinity_level: 0.21,
    labor_intensity_tolerance: 0.34,
    space_efficiency_preference: 0.59,
  },
  category_recommendations: [
    {
      rank: 1,
      score: 0.91,
      service_category_code: "CS100005",
      service_category_name: "제과점",
      category_group: "외식",
      stability_prior_score: 0.74,
      competition_pressure_score: 0.33,
      weekend_sales_ratio: 0.48,
      evening_sales_ratio: 0.39,
      late_night_sales_ratio: 0.07,
      female_sales_ratio: 0.61,
      franchise_ratio: 0.22,
    },
    {
      rank: 2,
      score: 0.82,
      service_category_code: "CS100004",
      service_category_name: "브런치카페",
      category_group: "카페",
      stability_prior_score: 0.67,
      competition_pressure_score: 0.44,
      weekend_sales_ratio: 0.58,
      evening_sales_ratio: 0.32,
      late_night_sales_ratio: 0.03,
      female_sales_ratio: 0.69,
      franchise_ratio: 0.18,
    },
    {
      rank: 3,
      score: 0.74,
      service_category_code: "CS100003",
      service_category_name: "샐러드전문점",
      category_group: "외식",
      stability_prior_score: 0.63,
      competition_pressure_score: 0.41,
      weekend_sales_ratio: 0.36,
      evening_sales_ratio: 0.28,
      late_night_sales_ratio: 0.02,
      female_sales_ratio: 0.72,
      franchise_ratio: 0.15,
    },
  ],
  created_at: "2026-06-22T14:30:00Z",
}

export const onboardingAreaRecommendationFixture: OnboardingAreaRecommendationResult =
  {
    result_code: onboardingResultFixture.result_code,
    selected_category_code: "CS100005",
    share_path: onboardingResultFixture.share_path,
    share_url: onboardingResultFixture.share_url,
    prediction: {
      trained_at: "2026-06-22T12:34:56Z",
      model_signature:
        "onboarding_two_tower:2026-06-22T12:34:56Z:minmax_zero_to_one_v1",
      top_k: 5,
      user_profile: {
        ...onboardingResultFixture.area_user_profile,
        preferred_category_code: "CS100005",
      },
      recommendations: [
        {
          rank: 1,
          score: 1,
          item_id: "11230536:CS100005",
          area_name: "망원2동",
          service_category_name: "제과점",
          area_profile_type: "residential",
          sales_amount: 1950000000,
          weekend_sales_ratio: 0.52,
          evening_sales_ratio: 0.22,
          resident_population: 23110,
          worker_population: 9110,
          subway_commercial_trend_score: 0.73,
          category_opportunity_score: 0.68,
          demand_gap_score: 0.59,
        },
        {
          rank: 2,
          score: 0.65,
          item_id: "11140587:CS100005",
          area_name: "연남동",
          service_category_name: "제과점",
          area_profile_type: "mixed",
          sales_amount: 2340000000,
          weekend_sales_ratio: 0.61,
          evening_sales_ratio: 0.29,
          resident_population: 18400,
          worker_population: 12300,
          subway_commercial_trend_score: 0.81,
          category_opportunity_score: 0.72,
          demand_gap_score: 0.45,
        },
        {
          rank: 3,
          score: 0.42,
          item_id: "11350612:CS100005",
          area_name: "성수1가",
          service_category_name: "제과점",
          area_profile_type: "office",
          sales_amount: 3120000000,
          weekend_sales_ratio: 0.38,
          evening_sales_ratio: 0.41,
          resident_population: 8900,
          worker_population: 31200,
          subway_commercial_trend_score: 0.89,
          category_opportunity_score: 0.61,
          demand_gap_score: 0.52,
        },
      ],
    },
  }
