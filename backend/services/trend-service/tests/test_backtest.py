...
def test_sign_flip_p_is_significant_for_strong_signal_not_noise() -> None:
...
def test_paired_perm_p_is_significant_when_model_beats_baseline() -> None:
    rng = np.random.default_rng(0)
...
def test_sign_flip_p_has_add_one_lower_bound() -> None:
    rng = np.random.default_rng(0)
...
def test_feature_label_screen_flags_leaky_feature() -> None:
    rng = np.random.default_rng(0)
    n = 300
    y = rng.normal(size=n)
...
def test_time_split_weeks_keeps_purge_gap() -> None:
...
def test_mean_week_ic_averages_cross_sectional_weekly_ic() -> None:
