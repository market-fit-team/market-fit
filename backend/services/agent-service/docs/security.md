...
`src/agent/security/auth.py`가 Better Auth JWT를 검증한다.  
...
프론트 쪽 JWT 경계는 `frontend/src/features/auth/lib/auth.ts`의 `jwt()` 플러그인과 `frontend/src/app/api/proxy/[...path]/route.ts`의 헤더 교체로 닫힌다.
