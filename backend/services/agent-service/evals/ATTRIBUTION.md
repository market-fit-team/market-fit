...
- `agent_eval/client.py`: `POST /api/v1/langgraph/threads`, SSE `POST /runs/stream`, 및 HITL resume를 위한 HTTP 클라이언트.
- `agent_eval/sse.py`: 이 저장소의 SSE 이벤트 규약(contract)을 위한 파서 및 헬퍼 함수.
