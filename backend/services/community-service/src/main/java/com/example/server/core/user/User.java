...
public class User {
    ...
    // 예: authentik, google
    // NOTE: 인증 provider 이름입니다. 런타임 backend 경계는 authentik JWT를 기준으로 합니다.
    @Column(nullable = false, length = 50) // NOTE: [JPA] DB 컬럼 설정 (null 불가, 길이 50)
    private String provider;

    // NOTE: provider 안에서 유일한 사용자 식별자입니다. authentik JWT 기준으로는 jwt.sub = authentik user id 입니다.
     ...
     * 현재 운영 흐름은 authentik JWT를 사용하므로 provider="authentik", providerSubject=jwt.sub 형태로 저장합니다.
     */
