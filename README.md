...
authentik을 이용한 JWT 발급 및 Traefik을 이용한 MSA 프로젝트입니다. 별도의 discovery 없이 docker compose의 라벨을 사용합니다.
...
- **인증서버**
  - authentik을 이용하여 OAuth 2.0 및 OpenID Connect 기반으로 JWT 토큰 발급 및 통합 인증을 처리합니다.
- **게이트웨이**
...
- backend/services/echo-service
  - 게이트웨이 내부에서 다른 서비스를 호출하는 예시가 포함되어 있습니다.
...
- JWTS
  ```
  {
    ...
    # ...
    "user_profile": {
      "uuid": "85d9ee80-7047-4011-b5be-e4150319f858",
      "display_name": "FastTutle3242",
      "age": null,
      "job": null,
      "avatar_seed": "3gsd41ag2"
    }
  }
  ```
  JWT를 파싱하면 user_profile.uuid가 담겨 있습니다. 이를 fk로 하여 사용자 정보와 연결합니다.
...
- **Permissions & HITL**
  ...
  - 관련 파일 :
    ...
    - [schemas.py](backend/services/agent-service/src/agent/services/chat/approvals/schemas.py) : 프론트엔드/백엔드 경계면으로 동작하며, HITL 요청 및 응답 데이터 구조(HitlRequest, HitlResume, HitlDecision 등)를 정의하고 요청을 받아 상태(state)에 반영하는 인터페이스 역할
